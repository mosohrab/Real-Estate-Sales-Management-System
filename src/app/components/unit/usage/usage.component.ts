import {
  Component, OnInit,
  Input, Output, ViewChild, ViewEncapsulation
} from '@angular/core';

import { State } from '@progress/kendo-data-query';
import {
  GridDataResult,
  DataStateChangeEvent,
  PageChangeEvent,
  SelectionEvent
} from '@progress/kendo-angular-grid';

import { UsageModel } from '../../../model/usage.model';
import { WeBaseKendoGridComponent } from '../../we-base-kendo-grid.component';
import { UsageService, UsageKendoGridService } from '../../../services/usage.service';
import { UsageDialogComponent } from './usage-dialog.component';

@Component({
  selector: 'app-usage',
  templateUrl: './usage.component.html',
  styleUrls: ['./usage.component.scss'],
  encapsulation: ViewEncapsulation.None,
  providers: [
    UsageService,
    UsageKendoGridService]

})
export class UsageComponent extends WeBaseKendoGridComponent {
  service: UsageKendoGridService;
  model = <UsageModel>{};
  // @ViewChild('statusDialogUpsert') statusDialogUpsert: SpecialStatusUpsertComponent;
  @ViewChild('statusDialog') statusDialog: UsageDialogComponent;

  constructor(service: UsageKendoGridService) {
    super(service);
    this.service = service;
  }

  ngOnInitHandler() {
    this._service.initBusyConfig(this.busyConfig);
    this.refresh();
  }

  private refresh(): void {
    this._service.readGrid();

  }
  private closedDialog(event: boolean) {
    if (event) {
      this.refresh();
    }

  }

  addClickedHandler() {
    this.statusDialog.openDialog();
  }

  editClickedHandler() {
    this.statusDialog.openDialogById(this.dataItemSelected[0]);
  }

  deleteClickedHandler() {
    const id = this.dataItemSelected[0];
    this._service.removeArrange(this.dataItemSelected);
  }


  deleteAllClickedHandler() {
    this._service.removeAll();
  }


}


