...
export class Tree {
  ...
  public parent: Tree;
  public nodeMenuSelected: NodeMenuItemSelectedEvent;
  // STATIC METHODS ----------------------------------------------------------------------------------------------------
  ...
  private static cloneTreeShallow(origin: Tree): Tree {
    ...
    tree.nodeMenuSelected = origin.nodeMenuSelected;
    return tree;
  }
   ...
   */
  public createNode(menu: NodeMenuItemSelectedEvent, isBranch: boolean, model: TreeModel = { value: '' }): Tree {

    const tree = new Tree(model, this, isBranch);
    tree.nodeMenuSelected = menu;
    if (!model.id) {
      tree.markAsNew();
    }
    ...
    if (this.isLeaf()) {
      result= this.addSibling(tree);
    } else {
      result= this.addChild(tree);
    }

    result.nodeMenuSelected = menu;
    return  result;
  }
  ...
  private _addChild(child: Tree, position: number = size(this._children) || 0): Tree {
    child.parent = this;
    child.nodeMenuSelected = this.nodeMenuSelected;
  ...
  public hasMenuItem(): boolean {
    return !get(this.node.settings, 'static', false) &&
      get(this.node.settings, 'menuItems', false);
  }
